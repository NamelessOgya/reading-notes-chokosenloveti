...
## 手法
GrIPSは、勾配を用いた数学的な最適化ではなく、「フレーズの書き換え」と「スコア評価」をサイクルとして回すことでプロンプトを改善していくヒューリスティックな探索手法である。

具体的には、以下のサイクル（探索ループ）を繰り返すことで最適な指示文を探し出す。

**1. 候補の生成（編集操作）**
人間向けに書かれた初期の指示文（プロンプト）を入力とし、CRFベースの構文解析器を用いてフレーズ単位に分割する。そこへ以下の4つの編集操作（Edit Operations）をランダムに組み合わせて適用し、複数の新たな指示文の候補（突然変異）を生成する。
...
**2. すべての候補をテスト（スコア計算）**
生成された複数の候補プロンプトを、小規模な評価データセット（スコアセット $\mathcal{S}$ 、通常100サンプル程度）に適用し、以下のスコア関数を用いて評価する。
ここで、モデルが常に同じ回答へ偏る（例えばすべて「はい」と答える等）のような意味の崩壊を防ぎ、出力の多様性を維持するペナルティとして、Balanced Accuracyに加えて予測結果の離散エントロピー $H$ を加味している。

$$ \mathrm{score} = \mathrm{BalancedAccuracy} + \alpha H $$

エントロピー $H$ は以下のように計算される（ $\mathcal{Y}$ はラベル空間、 $\hat{y}$ は予測、 $\alpha=10$ ）。式中の $p_{y}$ は、評価セット全体においてモデルが特定のラベル $y$ を予測した割合を意味する。

$$ H = \sum_{y \in \mathcal{Y}} -p_{y} \log(p_{y}) \text{ ; } p_{y} = \frac{1}{|\mathcal{S}|}\sum_{i=1}^{ |\mathcal{S}|}\mathbf{1}(\hat{y}_{i} = y) $$

**3. 最優秀候補の採用（自然淘汰と反復）**
テストした候補群の中で、最も高いスコアを獲得した候補を次の反復のベースラインとして採用（上書き）する（グリーディ探索、またはビーム探索）。
新しく採用されたプロンプトに対して、再度「1. 候補の生成」を行い、性能が向上しなくなるか、最大反復回数に到達するまでこの進化の実証サイクルを何度も繰り返す。
...
### 新規性
1. **APIベースのブラックボックス環境への適応と勾配計算の排除**
   これまでのプロンプトチューニング（Prefix-TuningやPrompt Tuningなど）はモデル内部の層に連続値のベクトルを挿入し、勾配を用いて最適化する「ホワイトボックス」でのアプローチが主流であったのに対し、GrIPSは、GPT-3のような内部パラメータや勾配が公開されていないAPIベースの大規模言語モデルに対しても適用可能な「完全なブラックボックス最適化（Gradient-free）」を実現した点に大きなブレークスルーがある。

2. **フレーズ単位の構文的な編集（離散空間の直接的な探索）**
   既存の手法では、指示文を一貫して人間が読める意味のある文章として扱うか、あるいは完全に連続的な数値ベクトルとして扱うかの二極化であった。GrIPSは、文章を「フレーズ単位の木構造」とみなし、DeleteやSwapといった離散的な文字列パターンの組み合わせ問題として最適化を定式化することで、連続値を使わずに効果的な探索を可能にした。

3. **「意味の崩壊」が学習性能を向上させるという発見（人間の直感との乖離）**
   GrIPSの探索によって得られた最適な指示文は、人間から見ると文法が破綻したり意味不明になってしまうケースが多々ある。それでも大幅な精度向上が得られるという事実から、「LLMの内部表現にとって数学的/計算上最適な自然言語指示」と「人間にとって読みやすく論理的な指示」は必ずしも一致しないという、非常に示唆に富む知見を提示した。これは大規模言語モデルに対する「意味」と「機能」の切り離しを指摘する結果である。
