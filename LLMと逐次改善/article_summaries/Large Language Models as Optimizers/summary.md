...
### 新規性
本論文（OPRO）の最も大きな新規性は、**「ブラックボックス・連続/離散を問わず、LLM自身を『汎用的なオプティマイザ（最適化アルゴリズム）』として直接活用するパラダイム（Optimization by PROmpting）を提示した点」**にある。以下に、その具体的な特徴や他のアプローチとの違いを整理する。

#### メタプロンプトへの「最適化履歴」の組み込みと暗黙の勾配推論
他のテキストベース最適化手法である「ProTeGi」と比較すると、アプローチの根本的な違いが浮き彫りになる。ProTeGiは、LLMに誤答例を分析させて**「なぜ間違えたか」という直接的なテキスト評価（明示的な勾配: Explicit Gradient）**を生成させ、それに沿ってプロンプトを修正させるという明示的・段階的な制御を採用している。
対してOPROは、「過去に生成したプロンプト案とその評価スコア」を昇順に並べてメタプロンプト内に含めるだけである。エラー分析などの手順を外側で構築せずとも、**LLMがスコアの上昇軌跡（Optimization trajectory）を読むだけで「どう書き換えればより点数が上がるか」という勾配（暗黙の勾配: Implicit Gradient）をコンテキスト内学習で勝手に推論し、自律的に更新方向を決定できる**ことを実証した。これは最適化プロセス全体を「LLMの内部推測」に委ねた独自のパラダイムである。

#### 複数履歴の俯瞰による大局的な進化（大局的 vs 局所的）
更新に用いる情報量（文脈）の手法も異なる。ProTeGiが「現在の1つのプロンプトといくつかのエラー例」を起点に次の一手の方向を決める局所的（マルコフ的）な修正プロセスであるのに対し、OPROは**「過去の全探索履歴から抽出した複数のプロンプト（トップ20個など）をスコア順に一列に並べてまとめて俯瞰させる」**。これにより、LLMは「プロンプトAよりBのほうが良く、さらにCへと進化したほうが良い」という全体的なグラデーション（進化のトレンド）を同時に比較しながら学習でき、局所探索ではなく大局的な最適化を志向していると言える。

#### 厳密な目的関数の定義が不要な汎用ブラックボックス最適化
微分不可能な問題や、数学的なソルバーでは形式化が困難なタスクであっても、自然言語による指示とスカラーの評価スコアさえあれば最適化ループを回せる。事実、線形回帰や巡回セールスマン問題（TSP）のような古典的な数学問題から、プロンプト言語自体の最適化にいたるまで、「評価スコアさえ計算できれば何でも最適化できる汎用オプティマイザ」としてLLMが機能することを示した汎用性の高さが画期的である。
