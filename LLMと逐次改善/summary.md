...
| :--- | :---: | :---: | :---: | :--- | :--- |
| Black box tuning for language model as a service | Black-Box Tuning (BBT) | 2022 | arXiv | API経由のLLMへの微細調整を可能にする、勾配情報を用いないDerivative-Free Optimizationによる連続プロンプト最適化。 | [詳細](<./article_summaries/Black box tuning for language model as a service/summary.md>) |
...
| Automatic Prompt Optimization with “Gradient Descent” and Beam Search | ProTeGi | 2023 | arXiv | 自然言語によるテキスト勾配を用いた勾配降下法とビーム探索を組み合わせ、方向性を持ったプロンプト最適化を行う手法。 | [詳細](<./article_summaries/Automatic Prompt Optimization with “Gradient Descent” and Beam Search/summary.md>) |
| Large Language Models as Optimizers | OPRO | 2023 | arXiv | LLMをブラックボックス最適化器として活用し、過去の評価スコアとプロンプト履歴からより良い解決策を探索するフレームワーク。 | [詳細](<./article_summaries/Large Language Models as Optimizers/summary.md>) |
...
| TextGrad: Automatic "Differentiation" via Text | TextGrad | 2024 | arXiv | LLMシステムの計算グラフ全体に対し、自然言語フィードバックをテキスト勾配（疑似勾配）として逆伝播させる汎用最適化。 | [詳細](<./article_summaries/TextGrad: Automatic "Differentiation" via Text/summary.md>) |
