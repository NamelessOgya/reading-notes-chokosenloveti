...
カテゴリは以下の5分類：**①制約付きデコード / ②データフィルタリング / ③パターンマッチング / ④トークナイゼーション / ⑤ガードレール**
...
| **Soldaini (2024)** | ② | 2024年 | ACL '24 | — | データ品質 | **Dolma**を提案。3兆トークンの事前学習コーパス構築において、正規表現によるHTMLタグ除去・PII検出・Boilerplate除去を第一防衛層として大規模適用。 |
| **Penedo (2024)** | ② | 2024年 | NeurIPS '24 | — | データ品質 | **FineWeb**を提案。15兆トークンのウェブコーパスで正規表現ヒューリスティックフィルタリングを事前段として使用。DataTroveライブラリで構築。 |
| **He (2024)** | ③ | 2024年 | NAACL '24 | — | 推論速度 | **REST**を提案。投機的デコードのドラフトモデルをデータストアのn-gramトライ検索に置き換え、ドラフトモデル不要で **1.62〜2.36倍** の高速化。 |
| **Kudo (2018)** | ④ | 2018年 | EMNLP '18 | — | トークン化 | **SentencePiece**を提案。BPEトークナイザーが必要とする正規表現による事前分割（Pre-tokenization）を排除し、生Unicodeテキストを直接処理。 |
| **Peek2 (2024)** | ④ | 2024年 | ACL '25 | — | トークン化速度 | tiktoken（`cl100k_base`）の正規表現事前分割がボトルネックであることを実測で証明し、regex-freeアルゴリズムで **2.48倍のスループット向上**。 |
| **Rebedea (2023)** | ⑤ | 2023年 | EMNLP '23 | — | 安全性 | **NeMo Guardrails**を提案。Colang言語で入出力ルールを記述し、正規表現・LLMベース分類器を組み合わせた多層防御をモデル再学習なしで実現。 |
| **Inan (2023)** | ⑤ | 2023年 | arXiv | — | 安全性 | **Llama Guard**を提案。Llama 2-7Bを安全性タクソノミーでファインチューニングした分類器。実用システムでは正規表現フィルター（第一層）→Llama Guard（第二層）の2層構成が推奨される。 |
