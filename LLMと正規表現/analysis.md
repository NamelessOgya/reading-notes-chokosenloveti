...
制約付きデコード研究は「**表現力の軸**（何で制約するか）」と「**効率の軸**（どう速く動かすか）」の2軸で整理できます。FSM/DFAとCFG/PDAはどちらを選ぶかの問題であり、高速化はどちらを選んでも必要になる共通課題です。

| 論文略称 | ＜表現力の軸＞<br>正規表現で制約<br>（FSM/DFA） | ＜表現力の軸＞<br>プログラム文法で制約<br>（CFG/PDA） | ＜効率の軸＞<br>マスク計算を<br>高速化する | ＜品質の問題＞<br>確率分布が<br>歪まないようにする | ＜品質の問題＞<br>推論能力を<br>損なわないようにする | ＜別方向の活用＞<br>正規表現を<br>LLMで生成する | ＜別方向の活用＞<br>正規表現で<br>LLMを解釈する |
| :--- | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
| **Willard (2023)** | ★ | ★ | ★ | | | | |
| **Ugare (2024a)** | ★ | ★ | ★ | | | | |
| **Dong (2024)** | ★ | ★ | ★ | ★ | | | |
...
> **読み方**: FSM/DFAとCFG/PDAは「どちらの文法クラスを使うか」の選択であり、高速化はその実装に共通して必要になる課題。多くの論文が表現力の軸と効率の軸に★を持つのはそのため。
...
│    └─ 推論能力の劣化      → CRANE                         │
├─────────────────────────────────────────────────────────┤
│  別方向のRE活用（制約付きデコードとは別の研究）                  │
│    ├─ LLMで正規表現を生成する  → NL2RE（InfeRE）            │
│    └─ 正規表現でLLMを解釈する → Semantic Regexes          │
└─────────────────────────────────────────────────────────┘
```
...
**制約付きデコードを使うと「出力の形式」は保証されるが、別の副作用が生じる。**

#### 確率歪み問題（Park 2024）

単純なトークンマスキングは「今のステップで文法違反かどうか」しか見ない。しかし「今は合法でも将来行き詰まるパス」を選ばせてしまい、元のLLMの確率比が崩れる。

```
正規表現: "hello" | "help"   （0.3 : 0.7 の確率でLLMは「help」を好む）
プレフィックス "hel" の時点:
  単純マスキング → "l" しか選べない → 次で "lo" か "p" か
  → この選択の確率が 0.3:0.7 から歪んでしまう
```

#### 推論能力の劣化（Banerjee 2025）

厳密な文法制約は中間推論ステップを禁止するため、LLMが Chain-of-Thought できなくなる。

```
制約付きで数学問題を解く:
  ❌ <number> 制約だと中間計算を書けない → 精度大幅低下
  ✅ CRANE: 推論フェーズは制約なし → 最終出力のみ制約 → 精度維持
```
...
制約付きデコードとは別に、正規表現をLLMと組み合わせる研究が2方向ある。

#### 正規表現を LLMで生成する（NL2RE）

自然言語の説明 → 正規表現を自動生成するタスク。InfeREは生成を連鎖推論に分解することで精度を向上させた。
```
入力: "1桁以上の数字のあとにスペース"
出力: [0-9]+\s
```

#### 正規表現で LLMを解釈する（Semantic Regexes）

LLMの内部特徴量（ニューロンが何に反応するか）を、意味的正規表現という構造化言語で記述する研究。自然言語説明より一貫性が高く定量化できる。
```
従来の自然言語説明: "フランス語のテキスト" → 曖昧
意味的正規表現:     CONTEXT(<lang:French>, <pos:VERB>) → 精確・定量可能
```
