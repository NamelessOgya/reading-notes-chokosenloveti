...
各列のテーマが意味する技術的内容を以下に解説します。

### 🔵 FSM/DFA制約デコード

**正規表現をFSM（有限状態機械）に変換し、LLMがトークンを選ぶ際に「現在のFSM状態から遷移できるトークン」だけを許可する手法。**

通常のLLMは全語彙の中から自由にトークンを選ぶが、FSM制約デコードでは各ステップで有効なトークン集合を計算し、それ以外のロジットを $-\infty$ でマスクする。

```
正規表現: \d{4}-\d{2}-\d{2}

生成ステップ1（状態0）: "2","1","0",... のみ許可
生成ステップ5（状態4）: "-" のみ許可
生成ステップ6（状態5）: "0","1" のみ許可
```

- **GPU正規表現との対応**: DFAで文字列を受け入れるか「判定」する処理を、LLMのトークン「選択」に転用した技術。構造は全く同じDFAを使う。
- **主な課題**: 語彙サイズ（数万〜数十万）×FSM状態数分のインデックスが必要 → DFA状態爆発問題と同種の問題。

---

### 🔵 CFG/PDA制約デコード

**プログラミング言語のような再帰的・入れ子構造を持つ文法（CFG）を制約として使う手法。**

正規表現（= 有限状態機械）はネストを表現できない（例：括弧の対応）が、CFGはできる。CFGに対応する計算機はプッシュダウンオートマトン（PDA）でスタックを持つ。
...
- **FSM制約との違い**: 正規表現 ⊂ CFG の表現力の関係があり、CFG制約の方がより汎用的だが計算コストが高い。
- **代表手法**: PICARD（Earleyパーサ）、SynCode（DFAマスクストア）、XGrammar（PDAとスタック管理）。

---

### 🔵 高速化・オーバーヘッド削減

**制約付きデコードの計算コストを減らし、実用的な推論速度を実現する手法群。**

制約なし生成に比べて、各ステップでのマスク計算や文法状態更新にCPUを使うため、GPU上のLLM計算とのボトルネックが発生する。

| 問題 | 解決アプローチ |
| :--- | :--- |
| マスク計算が毎ステップ $O(\|V\|)$ かかる | 事前インデックスで $O(1)$（Outlines） |
| CPUのマスク計算 vs GPU生成が直列 | GPU計算とCPUマスク計算をオーバーラップ（XGrammar） |
| サブワードとFSM境界のずれで正確なマスクが引けない | ボキャブラリー整合型ツリーで前処理（DOMINO） |

---

### 🔵 確率歪み問題

**「文法に合うトークンを選ぶ」という制約が、元のLLMの生成確率分布を歪めてしまう問題。**

単純なマスキングは「今のステップで文法違反かどうか」だけを見て無効トークンを除外する。しかし、現時点では有効でも**将来的に行き詰まるパス**を選ばせてしまうことがある。

```
正規表現: "hello"|"help"
プレフィックス "hel" まで生成 → 次は "l" しかない

しかし "hello"（0.3）と "help"（0.7）の確率比が、
単純マスキングでは正しく反映されない
```

- **本来あるべき姿**: 文法を満たす全完結形の上でLLMの確率分布を保存するサンプリング（GAD: Grammar-Aligned Decoding）。
- **実用解**: ASApアルゴリズムが過去サンプルを使って将来文法適合性を近似し補正する。

---

### 🔵 推論能力との両立

**文法制約が強すぎると、LLMの連鎖推論（Chain-of-Thought）能力が失われるという問題とその対策。**

CRANEが理論的に示した問題：
- CoT推論がLLMの能力を引き出す理由は、「中間ステップを外部に書き出す」ことで計算の深さを増やせるから。
- 厳密な文法制約は中間ステップの生成を禁止 → LLMが $TC^0$ クラスの計算しかできなくなる。

```
問題: "23 × 47 = ?" を直接 <number> 制約付きで生成
  → 中間計算を書けないので精度が大幅低下

解決（CRANE）:
  推論フェーズ: 制約なし → "23×47は20×47+3×47=940+141=1081"
  出力フェーズ: 制約あり → <number>1081</number>
```
...
**自然言語の説明からLLMが正規表現を自動生成する（NL2RE: Natural Language to Regular Expression）タスク。**

- **InfeRE**: 「1桁以上の数字の後にスペース」のような記述を、Chain of Inferenceでステップごとに `[0-9]+` → `\s` → 結合、という形で段階的に生成する。
- **Synchromesh**: 正規表現を生成する際に、正規表現の文法自体でLLMを制約することで、構文的に不正な正規表現が出力されるのを防ぐ。

---

### 🔵 LLM特徴量解釈

**LLMの内部表現（ニューロン・特徴量）が何を意味するかを正規表現で記述する研究。**

スパースオートエンコーダ（SAE）でLLMの活性化ベクトルを疎な単義的特徴量に分解した後、各特徴量が「どういうテキストパターンに反応するか」を記述するために意味的正規表現を使う。

```
自然言語説明（従来）: "フランス語のテキスト" → 曖昧
意味的正規表現（本手法）: CONTEXT(<lang:French>, <pos:VERB>) → 精確・定量可能
```
