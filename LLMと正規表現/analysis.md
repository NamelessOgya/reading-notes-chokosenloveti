...
| :--- | :---: | :---: | :---: | :---: | :---: | :--- |
| **Willard (2023)** | 制約付きデコード | 2023年 | arXiv | FSM/DFA | 速度 | **Outlines**を提案。正規表現→FSMに変換し、「状態→有効トークン集合」の事前インデックスでマスク計算を $O(N)$ から $O(1)$ に削減。制約付きデコードの事実上の標準ライブラリ。**GPU正規表現: ×**（マスク計算はCPUで実行） |
| **Ugare (2024a)** | 制約付きデコード | 2024年 | EMNLP '24 | CFG/DFA | 速度・正確性 | **SynCode**を提案。プログラミング言語のCFGからDFAマスクストアをオフラインで構築し、コード生成のシンタックスエラーを **96%削減**。健全性・完全性を理論保証。**GPU正規表現: ×**（DFAマスクストア参照はCPU） |
| **Dong (2024)** | 制約付きデコード | 2024年 | ICML '24 | FSM/CFG | 速度・正確性 | **DOMINO**を提案。サブワードトークンとFSM境界のずれ（Token Misalignment）問題を、ボキャブラリー整合型ツリーのオフライン構築で解決。投機的デコードと組み合わせ最大 **2倍のスループット**。**GPU正規表現: ×**（FSM処理はCPU） |
| **Dong (2024b)** | 制約付きデコード | 2024年 | arXiv | CFG/PDA | 速度 | **XGrammar**を提案。語彙を「状態非依存トークン（事前計算可）」と「状態依存トークン（実行時）」に分割し、GPU生成とCPUマスク計算を並行実行。従来比 **最大100倍の高速化**。vLLM等に採用。**GPU正規表現: ×**（GPUはLLM生成専用、マスク計算はCPUで並行実行） |
| **Park (2024)** | 制約付きデコード | 2024年 | NeurIPS '24 | FSM/CFG | 品質 | **GAD / ASAp**を提案。単純なトークンマスクは将来の文法適合性を無視するため元のLLM確率分布を歪める（確率歪み問題）ことを定式化し、過去サンプルを使った再重み付けで改善。**GPU正規表現: ×**（サンプリング計算はCPU） |
| **Banerjee (2025)** | 制約付きデコード | 2025年 | ICML '25 | FSM/CFG | 品質・推論 | **CRANE**を提案。厳密な文法制約はLLMの推論能力を理論的に劣化させる（$TC^0$制限）ことを証明。推論フェーズは制約なし・出力フェーズは制約ありに切り替える文法拡張デコードで数学・論理推論を **最大10ポイント改善**。**GPU正規表現: ×**（文法拡張ロジックはCPU） |
| **Boggust (2025)** | 制約付きデコード | 2025年 | ICLR '26 | — | 解釈可能性 | **Semantic Regexes**を提案。スパースオートエンコーダで得たLLM特徴量を「意味的正規表現」という構造化言語で記述。自然言語説明より一貫性が高く特徴量の複雑さを定量化できる。**GPU正規表現: —**（正規表現はLLM解釈のための記述言語であり実行対象ではない） |
| **Scholak (2021)** | 制約付きデコード | 2021年 | EMNLP '21 | CFG（SQL） | 正確性 | **PICARD**を提案。SQL生成において生成中トークンをEarleyパーサで逐次検証し、無効プレフィックスをビームから即時除外。学習追加なしでSpiderベンチマーク **SOTA（EM+12ポイント）** を達成。**GPU正規表現: ×**（EarleyパーサはCPUで実行） |
| **Poesia (2022)** | 制約付きデコード | 2022年 | ICLR '22 | FSM/CFG | 正確性・汎用性 | **Synchromesh**を提案。正規表現・CFGベースのCompletion Engineで各ステップの有効トークンを決定。DSL・化学式などタスク固有の形式に特化した制約でFew-shot精度を **3〜6倍** 改善。**GPU正規表現: ×**（Completion EngineはCPUで実行） |
| **Soldaini (2024)** | データフィルタリング | 2024年 | ACL '24 | — | データ品質 | **Dolma**を提案。3兆トークンの事前学習コーパス構築において、正規表現によるHTMLタグ除去・PII検出・Boilerplate除去を第一防衛層として大規模適用。**GPU正規表現: ×**（正規表現フィルタリングはCPU/マルチコア実行。GPU化余地が最大のユースケース） |
| **Penedo (2024)** | データフィルタリング | 2024年 | NeurIPS '24 | — | データ品質 | **FineWeb**を提案。15兆トークンのウェブコーパスで正規表現ヒューリスティックフィルタリングを事前段として使用。DataTroveライブラリで構築。**GPU正規表現: ×**（DataTroveのフィルタリングはCPUで実行） |
| **He (2024)** | パターンマッチング | 2024年 | NAACL '24 | — | 推論速度 | **REST**を提案。投機的デコードのドラフトモデルをデータストアのn-gramトライ検索に置き換え、ドラフトモデル不要で **1.62〜2.36倍** の高速化。**GPU正規表現: —**（n-gramトライ検索は正規表現ではなく前方一致検索） |
| **Kudo (2018)** | トークナイゼーション | 2018年 | EMNLP '18 | — | トークン化 | **SentencePiece**を提案。BPEトークナイザーが必要とする正規表現による事前分割（Pre-tokenization）を排除し、生Unicodeテキストを直接処理。**GPU正規表現: —**（正規表現を使わない設計が特徴） |
| **Peek2 (2024)** | トークナイゼーション | 2024年 | ACL '25 | — | トークン化速度 | tiktoken（`cl100k_base`）の正規表現事前分割がボトルネックであることを実測で証明し、regex-freeアルゴリズムで **2.48倍のスループット向上**。**GPU正規表現: —**（GPUではなくCPUアルゴリズム改善で正規表現を置換する対照的アプローチ） |
| **Rebedea (2023)** | ガードレール | 2023年 | EMNLP '23 | — | 安全性 | **NeMo Guardrails**を提案。Colang言語で入出力ルールを記述し、正規表現・LLMベース分類器を組み合わせた多層防御をモデル再学習なしで実現。**GPU正規表現: ×**（第一層の正規表現フィルタはCPUで実行） |
| **Inan (2023)** | ガードレール | 2023年 | arXiv | — | 安全性 | **Llama Guard**を提案。Llama 2-7Bを安全性タクソノミーでファインチューニングした分類器。実用システムでは正規表現フィルター（第一層）→Llama Guard（第二層）の2層構成が推奨される。**GPU正規表現: ×**（第一層の正規表現はCPU、第二層のLlama GuardはGPUで実行） |
